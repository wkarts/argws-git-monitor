...
import {
  AlertCircle, CheckCircle2, Clock3, ListChecks, RefreshCw, RotateCcw, ServerCrash,
  Wrench, XCircle
...
const overview = ref<QueueOverview>({
  ...
  worker_online: false, worker_count: 0, workers: [], worker_error: null
})
...
function progress(job: SyncJob): number | null {
...
async function load(silent = false): Promise<void> {
  ...
  errorMessage.value = ''
  try {
    const suffix = statusFilter.value ? `?status=${statusFilter.value}&limit=500` : '?limit=500'
...
async function retry(job: SyncJob): Promise<void> {
  try {
    const response = await api.post<SyncJob>(`/jobs/${job.id}/retry`)
    toasts.success('Job reenviado', `${response.label} foi colocado na fila.`)
    await load(true)
  } catch (error) {
    toasts.error('Não foi possível repetir', error instanceof ApiError ? error.message : undefined)
  }
}
...
async function reconcile(): Promise<void> {
  ...
  try {
    ...
    toasts.success('Fila reconciliada', response.message)
    await load(true)
  } catch (error) {
    toasts.error('Falha ao reconciliar', error instanceof ApiError ? error.message : undefined)
...
<template>
  <div class="page-stack queue-page">
    <section class="page-heading">
      <div><span class="eyebrow">PROCESSAMENTO ASSÍNCRONO</span><h2>Fila operacional</h2><p>Estado real do worker, importações, sincronizações e processamento dos repositórios.</p></div>
      <div class="button-row"><button class="button ghost" :disabled="reconciling" @click="reconcile"><Wrench :size="16" />Reconciliar travados</button><button class="button secondary" @click="load()"><RefreshCw :size="16" />Atualizar</button></div>
    </section>

    <section class="worker-state" :class="overview.worker_online ? 'online' : 'offline'">
      <div class="worker-icon"><CheckCircle2 v-if="overview.worker_online" :size="22" /><ServerCrash v-else :size="22" /></div>
      <div><strong>{{ overview.worker_online ? `Worker operacional · ${overview.worker_count}` : 'Worker Celery offline' }}</strong><p v-if="overview.worker_online">{{ overview.workers.join(', ') }}</p><p v-else>A fila não será processada enquanto o container worker não responder. {{ overview.worker_error || 'Verifique worker, RabbitMQ e credenciais do broker.' }}</p></div>
    </section>
    ...
    <section class="queue-toolbar">
      <div class="live-state"><i :class="{ active: overview.worker_online && activeCount > 0 }" /><strong>{{ !overview.worker_online ? 'Processamento interrompido · worker offline' : activeCount ? `${activeCount} processamento(s) ativo(s)` : 'Fila em repouso' }}</strong></div>
    ...
    <section class="queue-panel">
      ...
      <article v-for="job in jobs" v-else :key="job.id" class="job-row">
        ...
        <div class="job-main">
          ...
          <div v-if="progress(job) !== null" class="progress-track"><span :style="{ width: `${progress(job)}%` }" /></div>
          <div v-else-if="job.status === 'running'" class="progress-track indeterminate"><span /></div>
          <div class="job-meta"><span>{{ job.kind }}</span><span>Criado {{ formatDateTime(job.created_at) }}</span><span v-if="job.progress_total">{{ job.progress_current }}/{{ job.progress_total }}</span><span v-if="job.completed_at">Finalizado {{ formatDateTime(job.completed_at) }}</span></div>
        </div>
        <div class="job-actions"><button v-if="job.status === 'queued' || job.status === 'running'" class="button ghost compact danger-text" @click="cancel(job)">Cancelar</button><button v-if="job.status === 'failed' || job.status === 'cancelled'" class="button secondary compact" :disabled="!overview.worker_online" @click="retry(job)"><RotateCcw :size="14" />Repetir</button></div>
      </article>
    </section>
  </div>
</template>
...
<style scoped>
.worker-state{display:flex;gap:.8rem;align-items:center;padding:.85rem 1rem;border:1px solid var(--border);border-radius:1rem;background:var(--surface)}.worker-state.online{border-color:color-mix(in srgb,var(--success) 35%,var(--border));background:color-mix(in srgb,var(--success) 5%,var(--surface))}.worker-state.offline{border-color:color-mix(in srgb,var(--danger) 42%,var(--border));background:color-mix(in srgb,var(--danger) 6%,var(--surface))}.worker-icon{display:grid;place-items:center;width:2.7rem;height:2.7rem;border-radius:.8rem;color:var(--success);background:color-mix(in srgb,var(--success) 10%,var(--surface))}.offline .worker-icon{color:var(--danger);background:color-mix(in srgb,var(--danger) 10%,var(--surface))}.worker-state strong{color:var(--text-strong)}.worker-state p{margin:.12rem 0 0;color:var(--text-muted);font-size:.72rem}
