...
import {
  ...
  HardDrive, Play, RefreshCw, RotateCcw, Server, ShieldCheck, Trash2, TriangleAlert, X
...
const selectedProviderName = computed(()=>hubProviders.value.find(item=>item.id===manualForm.provider_id)?.name||providerMap.value[manualForm.provider_id]||'Destino automático')
...
function engineLabel(value:string|null|undefined):string{if(value==='minio')return'MinIO';if(value==='local_fallback')return'Local de contingência';if(value==='local')return'Local';return'Indisponível'}
...
function assignDefaults():void{
  ...
  const preferred=backupTargets.value.find(item=>item.system_default)?.id||backupTargets.value[0]?.id||providers.value[0]?.id||''
  ...
  if(!manualForm.provider_id||!backupTargets.value.some(item=>item.id===manualForm.provider_id))manualForm.provider_id=preferred
...
async function createBucket():Promise<void>{
  ...
  await run('bucket-create',async()=>{
    const created=await api.post<HubProvider>('/storage-hub/internal-buckets',{name:bucketName.value})
    const engine=engineLabel(created.engine)
    toasts.success('Bucket criado',`${created.bucket} está pronto em ${engine}.`)
    bucketName.value='projetos';await refreshStorage()
  })
}
async function deleteBucket(provider:HubProvider):Promise<void>{
  if(provider.system_default)return
  const confirmed=await dialogs.confirm({title:'Excluir bucket interno?',message:`${provider.bucket_alias||provider.bucket} será removido. A operação só é liberada se não houver objetos, snapshots ou políticas vinculadas.`,tone:'danger',confirmLabel:'Excluir bucket'})
  if(!confirmed)return
  await run(`bucket-delete-${provider.id}`,async()=>{await api.delete(`/storage-hub/internal-buckets/${provider.id}`);toasts.success('Bucket excluído');await refreshStorage()})
}
...
async function backupNow():Promise<void>{
  if(!manualForm.repository_id){await dialogs.message({title:'Selecione um repositório',message:'Escolha o projeto que será protegido antes de iniciar o backup.',tone:'warning'});return}
  const confirmed=await dialogs.confirm({title:'Iniciar backup agora?',message:`Repositório: ${selectedRepositoryName.value}\nDestino: ${selectedProviderName.value}\nConteúdo: ${manualForm.backup_type}`,tone:'info',confirmLabel:'Iniciar backup'})
  if(!confirmed)return
...
async function retention(policy:BackupPolicy):Promise<void>{const confirmed=await dialogs.confirm({title:'Aplicar retenção agora?',message:'Snapshots fora das regras de retenção poderão ser excluídos. Snapshots permanentes permanecem protegidos.',tone:'warning',confirmLabel:'Aplicar retenção'});if(!confirmed)return;await run(`retention-${policy.id}`,async()=>{const result=await api.post<{deleted:number}>(`/platform/backup-policies/${policy.id}/apply-retention`,{});toasts.success('Retenção aplicada',`${result.deleted} snapshot(s) removido(s).`);await load()})}
  ...
  <div class="backup-workspace page-stack">
    <section class="backup-hero">
      <div class="hero-copy">
        ...
        <h2>Backup local resiliente, buckets internos e réplica externa</h2>
        <p>O MinIO é preferencial. Se a topologia ainda não tiver o serviço disponível, o Git Monitor grava automaticamente no object store local persistente e mantém restore e réplica funcionando.</p>
      ...
      <div class="hero-metrics">
        ...
        <article :class="{degraded:usingFallback,bad:!storageOperational}"><span>STORAGE</span><strong>{{storageOperational?(usingFallback?'LOCAL':'ONLINE'):'OFFLINE'}}</strong><small>{{engineLabel(storageState?.engine)}}</small></article>
      </div>
    </section>

    <section v-if="storageOperational&&usingFallback" class="runtime-banner warn"><TriangleAlert :size="18"/><div><strong>MinIO indisponível, contingência local ativa</strong><p>Backups e buckets continuam funcionais em {{storageState?.fallback_path||'/data/backups/object-store'}}. Quando o MinIO voltar, novos snapshots podem voltar ao engine S3 sem perder os arquivos locais existentes.</p></div><button class="button ghost compact" :disabled="busy==='storage-refresh'" @click="refreshStorage"><RefreshCw :size="14"/>Testar MinIO</button></section>
    <section v-else-if="!storageOperational" class="runtime-banner bad"><TriangleAlert :size="18"/><div><strong>Storage interno indisponível</strong><p>{{storageState?.error||storageLoadError||'MinIO e armazenamento local não responderam.'}}</p></div><button class="button ghost compact" @click="refreshStorage">Testar novamente</button></section>
    <section v-else class="runtime-banner ok"><CheckCircle2 :size="18"/><div><strong>Object storage interno operacional</strong><p>MinIO disponível em rede privada; contingência local também permanece preparada.</p></div></section>
    ...
    <template v-if="tab==='backups'">
      <section class="workspace-card">
        <header><div><h3>Executar backup agora</h3><p>O destino é validado antes de o job entrar na fila. MinIO offline não bloqueia o backup se a contingência local estiver íntegra.</p></div><DatabaseBackup :size="20"/></header>
        ...
        <div class="backup-form-grid">
          ...
          <label><span>Destino</span><select v-model="manualForm.provider_id"><option value="">Automático</option><option v-for="provider in backupTargets" :key="provider.id" :value="provider.id">{{provider.name}} · {{engineLabel(provider.engine)}}</option></select></label>
    ...
    <template v-else-if="tab==='storage'">
      <section class="workspace-card">
        <header><div><h3>Buckets internos</h3><p>Namespaces gerenciados pelo Git Monitor. MinIO é preferencial e o storage local assume automaticamente quando necessário.</p></div><span class="engine-badge" :class="{warn:usingFallback}">{{engineLabel(storageState?.engine)}}</span></header>
      ...
      <section class="workspace-card">
        <header><div><h3>Integrações externas</h3><p>Redundância opcional para Dropbox, Google Drive, S3/MinIO externo ou SFTP. O backup interno não depende delas.</p></div><button class="button primary compact" @click="providerFormOpen=true"><CloudCog :size="14"/>Adicionar</button></header>
        <div v-if="!externalProviders.length" class="empty-panel">Nenhuma integração externa cadastrada.</div>
        <div v-else class="external-grid"><article v-for="provider in externalProviders" :key="provider.id" class="external-card"><div><strong>{{provider.name}}</strong><span>{{providerLabel(provider.kind)}}</span></div><div class="external-actions"><button class="button ghost compact" @click="testProvider(provider)">Testar</button><button class="button ghost compact danger" @click="deleteProvider(provider)"><Trash2 :size="13"/>Remover</button></div></article></div>
      </section>
