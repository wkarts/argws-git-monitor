...
function statusLabel(status: string): string {
  return ({
    ...
    legal_restriction: 'Restrição legal / DMCA',
    forbidden: 'Acesso administrativo negado',
    not_visible: 'Não visível pela API',
    unauthorized: 'Token não autorizado',
    error: 'Erro de diagnóstico'
...
async function load(): Promise<void> {
  ...
  try {
    ...
    toasts.error('Falha ao carregar conexões GitHub', error instanceof ApiError ? error.message : undefined)
  } finally {
    loading.value = false
  }
}
...
async function diagnose(): Promise<void> {
  if (!selectedConnection.value || !fullName.value.trim()) {
    toasts.warning('Informe a conexão e o repositório', 'Use o formato owner/repo.')
    return
  }
  busy.value = 'probe'
  probe.value = null
  ...
  try {
    ...
    toasts.error('Diagnóstico recusado', error instanceof ApiError ? error.message : undefined)
  } finally {
    busy.value = ''
  }
}
...
async function deleteRepository(): Promise<void> {
  ...
  const sure = window.confirm(
    `Excluir definitivamente ${target} da conta GitHub?\n\n` +
    'Esta operação não restaura nem contorna o bloqueio DMCA. Ela apenas solicita ao GitHub a exclusão da sua cópia/fork.'
  )
  if (!sure) return
  ...
  try {
    const result = await api.post<ToolResult>(
      ...
      { full_name: target, confirmation: confirmation.value }
    )
...
<template>
  <div class="page-stack compliance-page">
    <section class="page-heading">
      <div>
        <span class="eyebrow">CONFORMIDADE E LIMPEZA</span>
        <h2>Repositório bloqueado / DMCA</h2>
        <p>Remova da sua própria conta GitHub uma cópia ou fork que ficou indisponível por restrição legal, mesmo quando ele não aparece mais no catálogo do Git Monitor.</p>
      </div>
      <button class="button secondary" :disabled="loading" @click="load"><RefreshCw :size="16" />Atualizar conexões</button>
    </section>
    ...
    <section class="compliance-grid">
      <article class="tool-card">
        <header><Github :size="19" /><div><strong>1. Identificar a cópia</strong><span>Não precisa estar sendo monitorada</span></div></header>
        <label class="field"><span>Conexão GitHub</span><select v-model="selectedConnectionId" :disabled="loading"><option v-for="connection in connections" :key="connection.id" :value="connection.id">{{ connection.name }} · @{{ connection.github_login }}</option></select></label>
        <label class="field"><span>Repositório completo</span><input v-model="fullName" :placeholder="expectedOwner ? `${expectedOwner}/nome-do-fork` : 'owner/repo'" @keyup.enter="diagnose" /></label>
        <small v-if="expectedOwner" class="field-help">Por segurança, esta operação aceita apenas repositórios cujo owner seja <strong>@{{ expectedOwner }}</strong>.</small>
        <button class="button primary" :disabled="busy==='probe' || !selectedConnectionId || !fullName.trim()" @click="diagnose"><ShieldAlert :size="15" />Diagnosticar</button>
      </article>

      <article class="tool-card danger-zone">
        <header><Trash2 :size="19" /><div><strong>2. Exclusão definitiva</strong><span>Confirmação forte por owner/repo</span></div></header>
        <template v-if="probe">
          <div :class="['probe-status', statusTone(probe.status)]">
            <component :is="probe.status === 'accessible' ? CheckCircle2 : AlertTriangle" :size="18" />
            <div><strong>{{ statusLabel(probe.status) }}</strong><span>HTTP {{ probe.http_status ?? '—' }} · {{ probe.full_name }}</span></div>
          </div>
          <dl class="probe-details">
            <div><dt>Fork</dt><dd>{{ probe.fork === null ? 'não observável' : probe.fork ? 'sim' : 'não' }}</dd></div>
            <div><dt>Privado</dt><dd>{{ probe.private === null ? 'não observável' : probe.private ? 'sim' : 'não' }}</dd></div>
            <div><dt>Disabled</dt><dd>{{ probe.disabled === null ? 'não observável' : probe.disabled ? 'sim' : 'não' }}</dd></div>
            <div><dt>No Git Monitor</dt><dd>{{ probe.monitored_locally ? 'sim' : 'não' }}</dd></div>
          </dl>
