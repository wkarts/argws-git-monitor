...
def _safe_test_failure(request: Request, operation: str, exc: Exception) -> dict[str, Any]:
    logger.warning(
        "%s falhou; request_id=%s; error_type=%s",
        operation,
        _request_id(request),
        type(exc).__name__,
    )
    return {
        "ok": False,
        "message": "A validação falhou. Consulte a Central de Logs usando o correlation ID.",
...
async def _queue_job(
    ...
    *,
    ...
    try:
        ...
        logger.warning("Worker Celery indisponível: %s", type(exc).__name__)
        raise HTTPException(
            status_code=503,
            detail="Worker Celery indisponível. Consulte Sistema/Fila para diagnóstico.",
        ) from exc
    ...
    try:
        ...
        logger.exception("Falha ao enfileirar operação %s", kind)
        job.status = SyncJobStatus.FAILED
        job.error = f"enqueue_failed:{type(exc).__name__}"
        job.message = "Falha ao enviar operação ao worker."
        job.completed_at = datetime.now(UTC)
        await db.commit()
        raise HTTPException(
            status_code=503,
            detail="Falha ao enfileirar operação. Consulte a Central de Logs.",
        ) from exc
...
async def create_storage_provider(
    ...
    provider = StorageProvider(
        ...
        secret_encrypted=secret_store.encrypt_dict(payload.secret),
        secret_hint=secret_store.hint(payload.secret),
    ...
    await record_audit(
        ...
        details={"name": provider.name, "kind": provider.kind, "secret_hint": provider.secret_hint},
...
async def update_storage_provider(
    ...
    if payload.secret is not None:
        store = SecretStore()
        provider.secret_encrypted = store.encrypt_dict(payload.secret)
        provider.secret_hint = store.hint(payload.secret)
    if payload.enabled is not None:
        provider.enabled = payload.enabled
    await record_audit(
        db,
        action="storage_provider.updated",
        user_id=current_user.id,
        entity_type="storage_provider",
        entity_id=str(provider.id),
        details={
            "name": provider.name,
            "kind": provider.kind,
            "secret_changed": payload.secret is not None,
        },
        ip_address=_client_ip(request),
    )
    await db.commit()
...
async def delete_storage_provider(
    ...
    usage = await db.scalar(
        select(func.count()).select_from(BackupPolicy).where(BackupPolicy.provider_id == provider.id)
    )
    if usage:
        raise HTTPException(
            status_code=409,
            detail="Provider possui políticas de backup vinculadas; remova/reassocie as políticas primeiro.",
        )
    await record_audit(
        db,
        ...
        entity_type="storage_provider",
        entity_id=str(provider.id),
        details={"name": provider.name, "kind": provider.kind},
        ip_address=_client_ip(request),
    )
...
async def test_storage_provider(
    ...
    try:
        ...
        return StorageProviderTestResult(
            ok=True,
            message="Provider validado com sucesso.",
            details=details,
        )
    except Exception as exc:
        return StorageProviderTestResult(**_safe_test_failure(request, "storage_provider.test", exc))
...
async def list_backup_policies(
    ...
    if repository_id:
        query = query.where(BackupPolicy.repository_id == repository_id)
...
async def run_backup_policy(
    ...
    queued = await _queue_job(
        db,
        current_user=current_user,
        repository=repository,
        kind="repository.backup",
        label=f"Backup · {repository.full_name} · {policy.name}",
        total=5,
        task=backup_task,
        params={"user_id": str(current_user.id), "policy_id": str(policy.id)},
    )
    await record_audit(
        db,
        ...
        entity_type="backup_policy",
        entity_id=str(policy.id),
        details={"repository": repository.full_name, "job_id": queued["job_id"]},
        ip_address=_client_ip(request),
    )
    await db.commit()
    return queued
...
async def run_backup(
    ...
    _require_operation(current_user, "backup.execute")
    if not payload.repository_id or not payload.provider_id or not payload.backup_type:
        raise HTTPException(
            status_code=422,
            detail="repository_id, provider_id e backup_type são obrigatórios no backup manual.",
        )
    repository = await _repository(db, payload.repository_id, current_user.id)
    provider = await db.get(StorageProvider, payload.provider_id)
    if not provider or provider.user_id != current_user.id:
    ...
    queued = await _queue_job(
        ...
        current_user=current_user,
        repository=repository,
        kind="repository.backup.manual",
        label=f"Backup manual · {repository.full_name}",
        total=5,
        task=backup_task,
        params={
...
async def list_backups(
    ...
    if repository_id:
        query = query.where(BackupSnapshot.repository_id == repository_id)
    return list(
        (await db.execute(query.order_by(BackupSnapshot.created_at.desc()).limit(limit)))
        .scalars()
        .all()
    )
...
async def apply_policy_retention(
    policy_id: uuid.UUID,
    current_user: CurrentUser,
    db: DbSession,
    request: Request,
    ...
    if not policy or policy.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Política não encontrada.")
    result = await apply_retention(db, policy)
    await record_audit(
        db,
        action="backup.retention_applied",
        user_id=current_user.id,
        entity_type="backup_policy",
        entity_id=str(policy.id),
        details=result,
...
async def queue_restore(
    ...
    queued = await _queue_job(
        ...
        label=f"Restore · {repository.full_name}",
        total=4,
        task=restore_task,
        params={
            "user_id": str(current_user.id),
    ...
    return list(
        (
            await db.execute(
                ...
                .where(PublishingChannel.user_id == current_user.id)
                .order_by(PublishingChannel.name)
            )
        )
        .scalars()
        .all()
...
async def create_publishing_channel(
    ...
    channel = PublishingChannel(
        user_id=current_user.id,
        secret_encrypted=store.encrypt_dict(payload.secret),
        **data,
    )
    db.add(channel)
    await db.flush()
    await record_audit(
        db,
        action="publishing_channel.created",
        user_id=current_user.id,
        entity_type="publishing_channel",
...
async def queue_release(
    ...
    repository = await _repository(db, payload.repository_id, current_user.id)
    params = {**payload.model_dump(mode="json"), "user_id": str(current_user.id)}
    queued = await _queue_job(
        db,
        current_user=current_user,
        repository=repository,
        kind="release.publish",
        label=f"Release {payload.tag_name} · {repository.full_name}",
        total=4,
        task=release_task,
        params=params,
    )
    await record_audit(
        ...
        details={
            ...
            "channels": [str(x) for x in payload.channel_ids],
            "job_id": queued["job_id"],
        },
        ip_address=_client_ip(request),
    )
...
async def queue_replication(
    payload: ReplicationRequest,
    current_user: CurrentUser,
    db: DbSession,
    request: Request,
) -> dict[str, Any]:
    _require_operation(current_user, "repository.replicate")
    repository = await _repository(db, payload.repository_id, current_user.id)
    queued = await _queue_job(
        db,
        current_user=current_user,
        repository=repository,
        kind="repository.replicate",
        label=f"Replicação · {repository.full_name}",
        total=3,
        task=replicate_task,
        params={**payload.model_dump(mode="json"), "user_id": str(current_user.id)},
    )
    await record_audit(
        db,
        action="repository.replication_queued",
        user_id=current_user.id,
        entity_type="repository",
        entity_id=str(repository.id),
        details={
...
async def list_deployment_targets(
    ...
    result = await db.execute(
        ...
        .where(DeploymentTarget.user_id == current_user.id)
        .order_by(DeploymentTarget.environment, DeploymentTarget.name)
    )
    return list(result.scalars().all())
...
async def create_deployment_target(
    payload: DeploymentTargetCreate,
    current_user: CurrentUser,
    db: DbSession,
    request: Request,
) -> DeploymentTarget:
    ...
    if payload.repository_id:
        await _repository(db, payload.repository_id, current_user.id)
    store = SecretStore()
    data = payload.model_dump(exclude={"secret"})
...
async def test_deployment_target(
    ...
    try:
        return await asyncio.to_thread(lambda: asyncio.run(test_target(target)))
    except Exception as exc:
        return _safe_test_failure(request, "deployment_target.test", exc)
...
async def queue_deploy(
    ...
    _require_operation(current_user, "deploy.execute")
    target = await db.get(DeploymentTarget, target_id)
    if not target or target.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Target não encontrado.")
    repository = await _repository(db, payload.repository_id, current_user.id)
    expected = f"DEPLOY {target.name} {payload.ref}"
    ...
    queued = await _queue_job(
        ...
        label=f"Deploy {target.environment} · {repository.full_name} · {payload.ref}",
        total=7,
        task=deploy_task,
        params={
            **payload.model_dump(mode="json"),
...
async def list_deployments(
    ...
    result = await db.execute(
        ...
        .where(DeploymentRecord.user_id == current_user.id)
...
async def queue_rollback(
    ...
    _require_operation(current_user, "deploy.rollback")
    record = await db.get(DeploymentRecord, deployment_id)
    if not record or record.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Deployment não encontrado.")
    repository = (
        await _repository(db, record.repository_id, current_user.id)
    ...
    await record_audit(
        ...
        user_id=current_user.id,
        entity_type="deployment_record",
        entity_id=str(record.id),
        details={"job_id": queued["job_id"]},
        ip_address=_client_ip(request),
    )
    await db.commit()
    return queued
...
async def queue_clinic(
    payload: ClinicRunRequest,
    current_user: CurrentUser,
    db: DbSession,
) -> dict[str, Any]:
    repository = await _repository(db, payload.repository_id, current_user.id)
    return await _queue_job(
        ...
        current_user=current_user,
        repository=repository,
        kind="repository.clinic",
        label=f"Clínica · {repository.full_name}",
        total=6,
        task=clinic_task,
        params={**payload.model_dump(mode="json"), "user_id": str(current_user.id)},
    )
...
async def _clinic_payload(db: DbSession, analysis: ClinicAnalysis) -> ClinicAnalysisRead:
    findings = list(
        (
            await db.execute(
                select(ClinicFinding)
                .where(ClinicFinding.analysis_id == analysis.id)
                .order_by(ClinicFinding.severity.desc())
            )
        )
        .scalars()
        .all()
    )
    payload = ClinicAnalysisRead.model_validate(analysis)
    payload.findings = [ClinicFindingRead.model_validate(item) for item in findings]
    return payload


@router.get("/clinic", response_model=list[ClinicAnalysisRead])
async def list_clinic(
    current_user: CurrentUser,
    db: DbSession,
    repository_id: uuid.UUID | None = None,
    limit: int = Query(default=30, ge=1, le=100),
) -> list[ClinicAnalysisRead]:
    query = select(ClinicAnalysis).where(ClinicAnalysis.user_id == current_user.id)
    if repository_id:
        query = query.where(ClinicAnalysis.repository_id == repository_id)
    analyses = list(
        (
            await db.execute(query.order_by(ClinicAnalysis.created_at.desc()).limit(limit))
        )
        .scalars()
        .all()
    )
    return [await _clinic_payload(db, item) for item in analyses]
...
async def list_cleanup_profiles(
    ...
    if repository_id:
        query = query.where(CleanupProfile.repository_id == repository_id)
...
async def create_cleanup_profile(
    payload: CleanupProfileCreate,
    current_user: CurrentUser,
    db: DbSession,
    request: Request,
) -> CleanupProfile:
    _require_operation(current_user, "cleanup.profile.manage")
    repository = await _repository(db, payload.repository_id, current_user.id)
    profile = CleanupProfile(user_id=current_user.id, **payload.model_dump())
    db.add(profile)
    await db.flush()
    await record_audit(
        db,
        action="cleanup.profile.created",
        user_id=current_user.id,
        entity_type="cleanup_profile",
        entity_id=str(profile.id),
        details={"repository": repository.full_name, "name": profile.name},
        ip_address=_client_ip(request),
    )
    await db.commit()
...
async def queue_cleanup_analysis(
    ...
    return await _queue_job(
        ...
        current_user=current_user,
        repository=repository,
        kind="repository.cleanup.analyze",
        label=f"Cleanup Analyze · {repository.full_name}",
        total=5,
        task=cleanup_analyze_task,
        params={**payload.model_dump(mode="json"), "user_id": str(current_user.id)},
    )
    ...
    candidates = list(
        (
            await db.execute(
                ...
                .where(CleanupCandidate.analysis_id == analysis.id)
                .order_by(CleanupCandidate.resource_type, CleanupCandidate.resource_key)
            )
        )
        .scalars()
        .all()
    )
    payload = CleanupAnalysisRead.model_validate(analysis)
    payload.candidates = [CleanupCandidateRead.model_validate(item) for item in candidates]
    return payload
...
async def list_cleanup(
    ...
    repository_id: uuid.UUID | None = None,
    limit: int = Query(default=30, ge=1, le=100),
) -> list[CleanupAnalysisRead]:
    query = select(CleanupAnalysis).where(CleanupAnalysis.user_id == current_user.id)
    if repository_id:
        query = query.where(CleanupAnalysis.repository_id == repository_id)
    analyses = list(
        (
            await db.execute(query.order_by(CleanupAnalysis.created_at.desc()).limit(limit))
        )
        .scalars()
        .all()
    )
    return [await _cleanup_payload(db, item) for item in analyses]
...
async def cleanup_selection(
    analysis_id: uuid.UUID,
    payload: CleanupSelectionRequest,
    current_user: CurrentUser,
    db: DbSession,
) -> CleanupAnalysisRead:
    analysis = await set_cleanup_selection(
        db,
        user_id=current_user.id,
...
async def queue_cleanup_dry_run(
    analysis_id: uuid.UUID,
    current_user: CurrentUser,
    db: DbSession,
) -> dict[str, Any]:
    ...
    if not analysis or analysis.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Análise não encontrada.")
    repository = await _repository(db, analysis.repository_id, current_user.id)
    return await _queue_job(
        db,
        current_user=current_user,
        repository=repository,
        kind="repository.cleanup.dry_run",
        label=f"Dry Run · {analysis.reference}",
        total=4,
        task=cleanup_dry_run_task,
        params={"user_id": str(current_user.id), "analysis_id": str(analysis.id)},
    )


@router.post("/cleanup/{analysis_id}/execute")
async def queue_cleanup_execute(
    analysis_id: uuid.UUID,
    ...
    _require_operation(current_user, "cleanup.execute")
    analysis = await db.get(CleanupAnalysis, analysis_id)
    if not analysis or analysis.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Análise não encontrada.")
    repository = await _repository(db, analysis.repository_id, current_user.id)
        ...
        (
            await db.execute(
                select(CleanupCandidate).where(
                    CleanupCandidate.analysis_id == analysis.id,
                    CleanupCandidate.selected.is_(True),
                    CleanupCandidate.protected.is_(False),
                )
            )
        )
        .scalars()
    ...
    destructive = any(item.action_class == ActionClass.DESTRUCTIVE.value for item in selected)
    backup_snapshot_id = None
    if destructive:
        if not payload.create_backup:
            raise HTTPException(
                status_code=409,
                detail="Ações DESTRUCTIVE exigem backup prévio; create_backup=false foi recusado.",
            )
        snapshot = (
            await db.execute(
                select(BackupSnapshot)
                .where(
                    BackupSnapshot.repository_id == repository.id,
        ...
        if not snapshot:
            raise HTTPException(
                status_code=409,
                detail=(
                    "Nenhum backup concluído existe para este repositório. "
                    "Execute Backup Now e repita o Dry Run antes da limpeza destrutiva."
                ),
            )
    ...
    queued = await _queue_job(
        ...
        current_user=current_user,
        repository=repository,
        kind="repository.cleanup.execute",
        label=f"Cleanup Execute · {analysis.reference}",
        total=12,
        task=cleanup_execute_task,
        params={
            "user_id": str(current_user.id),
    ...
    await record_audit(
        db,
        action="cleanup.execution_queued",
        user_id=current_user.id,
        entity_type="cleanup_analysis",
...
async def cleanup_report(
    ...
    if not analysis or analysis.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Análise não encontrada.")
    payload = (await _cleanup_payload(db, analysis)).model_dump(mode="json")
    return Response(
        content=json.dumps(payload, ensure_ascii=False, indent=2),
        media_type="application/json",
        headers={"Content-Disposition": f'attachment; filename="{analysis.reference}.json"'},
    )
...
async def list_audit(
    ...
    if operation_prefix:
        query = query.where(AuditLog.action.startswith(operation_prefix))
    rows = (
    ...
    for item in rows:
        writer.writerow(
            [
                ...
                item.entity_id or "",
                item.ip_address or "",
                json.dumps(item.details or {}, ensure_ascii=False, separators=(",", ":")),
            ]
        )
    return Response(
        content=output.getvalue(),
        media_type="text/csv; charset=utf-8",
        headers={"Content-Disposition": 'attachment; filename="argws-git-monitor-audit.csv"'},
    )
...
async def operations_dashboard(
    ...
    repositories = await db.scalar(
        ...
        .where(Repository.connection.has(user_id=current_user.id))
    ) or 0
    backups_success = await db.scalar(
        select(func.count())
        .select_from(BackupSnapshot)
        .where(
    ...
    backups_failed = await db.scalar(
        select(func.count())
        .select_from(BackupSnapshot)
        .where(
            BackupSnapshot.user_id == current_user.id,
    ...
    deployments_pending = await db.scalar(
        select(func.count())
        .select_from(DeploymentRecord)
        .where(
            DeploymentRecord.user_id == current_user.id,
            DeploymentRecord.status.in_(["queued", "running", "validating"]),
        )
    ) or 0
    deployments_failed = await db.scalar(
        select(func.count())
        .select_from(DeploymentRecord)
        .where(
            DeploymentRecord.user_id == current_user.id,
    ...
    critical = await db.scalar(
        select(func.count())
        .select_from(ClinicFinding)
        .join(ClinicAnalysis, ClinicFinding.analysis_id == ClinicAnalysis.id)
        .where(
            ClinicAnalysis.user_id == current_user.id,
            ClinicFinding.severity == "critical",
        )
    ) or 0
    latest_backup = (
        await db.execute(
    ...
    return {
        "repositories_monitored": repositories,
        "backups_successful": backups_success,
        "backups_failed": backups_failed,
        "storage_consumption_bytes": int(storage),
        "pending_deployments": deployments_pending,
        "failed_deployments": deployments_failed,
        ...
        "latest_deployments": [
            {
                ...
                "status": item.status,
                "ref": item.requested_ref,
                "created_at": item.created_at.isoformat(),
            }
            for item in latest_deploy
        ],
    }
